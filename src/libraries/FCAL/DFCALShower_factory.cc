...
DFCALShower_factory::DFCALShower_factory()
{
  ...
  FCAL_RADIATION_LENGTH[0] = 0;
  FCAL_CRITICAL_ENERGY[0] = 0;
  FCAL_SHOWER_OFFSET[0] = 0;
	
  gPARMS->SetDefaultParameter("FCAL:FCAL_RADIATION_LENGTH", FCAL_RADIATION_LENGTH[0]);
  gPARMS->SetDefaultParameter("FCAL:FCAL_CRITICAL_ENERGY", FCAL_CRITICAL_ENERGY[0]);
  gPARMS->SetDefaultParameter("FCAL:FCAL_SHOWER_OFFSET", FCAL_SHOWER_OFFSET[0]);
...
jerror_t DFCALShower_factory::brun(JEventLoop *loop, int32_t runnumber)
{
      ...
      if(loop->GetCalib("FCAL/depth_correction_params", depth_correction_params)) {
         ...
         FCAL_RADIATION_LENGTH[0] = depth_correction_params["radiation_length"];
         FCAL_CRITICAL_ENERGY[0]  = depth_correction_params["critical_energy"];
         FCAL_SHOWER_OFFSET[0] = depth_correction_params["shower_offset"];
      }
  ...
  if (fcal_parms.find("FCAL_C_EFFECTIVE")!=fcal_parms.end()){
    ...
    if(debug_level>0)jout<<"FCAL_C_EFFECTIVE = "<<FCAL_C_EFFECTIVE[0]<<endl;
  } else {
    jerr<<"Unable to get FCAL_C_EFFECTIVE from FCAL/fcal_parms in Calib database!"<<endl;
  }

  // Some parameters for FCAL insert (eventually these should go to ccdb)
  ...
  if (geom) {
    ...
    vector<double>LG_stacking_unit;
    geom->Get("//box[@name='LGBU']/@X_Y_Z",LG_stacking_unit);
    double midz=m_FCALfront[0]+0.5*LG_stacking_unit[2];

    vector<double>block;
    geom->Get("//box[@name='LGBL']/@X_Y_Z",block);
    m_FCALback[0]=m_FCALfront[0]+block[2];

    geom->Get("//box[@name='LTB1']/@X_Y_Z",block);
    vector<double>insert_center;
