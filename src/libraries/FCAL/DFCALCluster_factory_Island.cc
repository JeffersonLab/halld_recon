...
jerror_t DFCALCluster_factory_Island::evnt(JEventLoop *loop, uint64_t eventnumber)
{
  ...
  for (size_t i=0;i<fcal_hits.size();i++){
    ...
    hits.push_back(HitInfo(myhit->row,myhit->column,myhit->E,myhit->x,myhit->y,
			   myhit->t));
  }
  for (size_t i=0;i<ecal_hits.size();i++){
    ...
    hits.push_back(HitInfo(row,col,myhit->E,pos.X(),pos.Y(),myhit->t));
  }
  ...
  for (unsigned int i=0;i<clusterCandidates.size();i++){
	...
	myCluster->addHit(channel,E,clusterCandidate[k].x,
			  clusterCandidate[k].y,clusterCandidate[k].t);
      }
      x/=Etot;
      y/=Etot;
      t/=Etot;
	...
	if (myPeak.E>MIN_CLUSTER_SEED_ENERGY){
	  ...
	  if (good_fit){// && chisq/ndf<chisq_old/ndf_old){
	    ...
	    //cout << " Adding guess " << chisq/ndf <<"  " << chisq_old/ndf_old <<  endl;
...
bool DFCALCluster_factory_Island::FitPeaks(const TMatrixD &W,double b,
  ...
  //_DBG_ << "Guess " << myNewPeak.E << " " << myNewPeak.x << " " << myNewPeak.y << endl;
  ...
  for (unsigned int k=0;k<max_iter;k++){
    ...
    if (myNewPeak.E<0){ 
      //_DBG_ << myNewPeak.E << " " << myNewPeak.x << " " << myNewPeak.y << endl;
	...
	// Make sure the energy is positive!
	if (myPeakInfo.E<0){
	  //_DBG_ << myPeakInfo.E << " " << myPeakInfo.x << " " << myPeakInfo.y << endl;
