...
jerror_t DFDCPseudo_factory::brun(JEventLoop *loop, int32_t runnumber)
{
  ...
  double myrot[24]={180,120,60,0,-60,-120, 180,120,60,0,-60,-120,
  ...
  if (jcalib->Get("FDC/cell_offsets",vals)==false){
    for(unsigned int i=0; i<vals.size(); i++){
      ...
      double u=row["xshift"];
      double v=row["yshift"];
      double cosa=cos(myrot[i]*M_PI/180);
      double sina=sin(myrot[i]*M_PI/180.);
      double x=u*cosa+v*sina;
      double y=-u*sina+v*cosa;
      //_DBG_ << row["xshift"] << " " <<  row["yshift"] << " " << x << " " << y << endl;
      sumx+=x;
      sumy+=y;
    }
  }
  _DBG_ << sumx/24. << " " << sumy/24. << endl;
