...
public :
  ...
  jerror_t FindCentroid(DMatrix3x1 N, DMatrix3x1 X, double &pos,
			double *sum_f=NULL,double *q_ph=NULL){

    const unsigned int X0=0;
    const unsigned int QA=1;
    const unsigned int K2=2;
    const int ITER_MAX=100;
    const float TOLX=1e-4;
    const float TOLF=1e-4;
    const float A_OVER_H=0.4;
    const float ONE_OVER_H=2.0;
  ...
  jerror_t FindNewParmVec(const DMatrix3x1 &N,
    ...
    const float A_OVER_H=0.4;
    const float ONE_OVER_H=2.0;
    const float ALPHA=1e-4; // rate parameter for Newton step backtracking algorithm
