	...
	japp->RootWriteLock(); //ACQUIRE ROOT LOCK!! //I have no idea why this is needed (for the cuts), but without it it crashes.  Sigh. 
	{
		...
		for(const auto& locPIDPair : dMissingMassSquaredCuts)
		{
			...
			if(locHist == nullptr)
			{
				...
				auto locNumBins = 1000.0*(locMaxMass - locMinMass);
				dHistMap_MissingMassSquaredVsBeamEnergy[locPID] = (TH2*)new TH2I(locHistName.c_str(), locHistTitle.c_str(), 600, 0.0, 12.0, locNumBins, locMinMass, locMaxMass);
			}
			else
		...
		//NONE MISSING, MISSING E:
		{
			...
			if(locHist == nullptr)
			{
				...
				dHist_NoneMissing_MissingEVsBeamEnergy_PreMissMassSqCut = (TH2*)new TH2I(locHistName.c_str(), locHistTitle.c_str(), 600, 0.0, 12.0, 1200, -6.0, 6.0);
			}
			else
			...
			{
				string locHistTitle = string("None Missing: From All Production Mechanisms;Beam Energy (GeV); Missing E (GeV)");
				dHist_NoneMissing_MissingEVsBeamEnergy_PostMissMassSqCut = (TH2*)new TH2I(locHistName.c_str(), locHistTitle.c_str(), 600, 0.0, 12.0, 1200, -6.0, 6.0);
			}
			else
		...
		//NONE MISSING, MISSING PT:
		{
			...
			if(locHist == nullptr)
			{
				...
				dHist_NoneMissing_MissingPtVsMissingE_PreMissMassSqCut = (TH2*)new TH2I(locHistName.c_str(), locHistTitle.c_str(), 1200, -6.0, 6.0, 800, 0.0, 4.0);
			}
			else
			...
			{
				string locHistTitle = string("None Missing: From All Production Mechanisms; Missing E (GeV); Missing Transverse Momentum (GeV/c)");
				dHist_NoneMissing_MissingPtVsMissingE_PostMissMassSqCut = (TH2*)new TH2I(locHistName.c_str(), locHistTitle.c_str(), 1200, -6.0, 6.0, 600, 0.0, 3.0);
			}
			else
