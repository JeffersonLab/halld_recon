...
{   
  ...
  for(unsigned int i=0; i<digihits.size(); i++){
    ...
    for (unsigned int j=0;j<tdcdigihits.size();j++){
      const DCTOFTDCDigiHit *tdcdigihit = tdcdigihits[i];
      
      if (tdcdigihit->bar!=digihit->bar && tdcdigihit->end!=digihit->end) 
	continue;

      // pulse pedestal
      double pedestal=0.,pedestal4Amp=0.;
	...
	pedestal4Amp = digihit->pedestal/double(nsamples_pedestal);
      ...
      double Tadc = t_scale*double(digihit->pulse_time) + adc_time_offsets[ind] + t_base;
