...
void DBeamHelicity_factory::Init()
{
  ...
  if(app){                                              //override with command line if needed
    app->SetDefaultParameter("HELIF:VERBOSE", HELIVERBOSE, "Set level of verbosity on helicity");
    app->SetDefaultParameter("HELIF:SETUP",   HELISETUP,   "Set name of helicity setup file.");
    app->SetDefaultParameter("HELIF:LOG",     HELILOG,     "Set name of helicity log file.");
  }
...
void DBeamHelicity_factory::Process(const std::shared_ptr<const JEvent>& event){
  ...
  locBeamHelicity->real_hel      = 0;
  locBeamHelicity->valid         = 0;  //assume real_hel not valid unless we find out otherwise.  
  ...
  std::cout <<  eventnumber <<  " " << locBeamHelicity->helicity << "  " << locBeamHelicity->pattern_sync <<  " " << locBeamHelicity->pair_sync << endl;
  ...
  f_pattern_sync = locBeamHelicity->pattern_sync;                //save local copies
  f_t_settle     = locBeamHelicity->t_settle;
  f_helicity     = locBeamHelicity->helicity;
  f_pair_sync    = locBeamHelicity->pair_sync;
  f_ihwp         = locBeamHelicity->ihwp;
  f_beam_on      = locBeamHelicity->beam_on;
  ...
  if(fHelicity == -1){
    locBeamHelicity->valid    = 1;
    locBeamHelicity->real_hel = 0;
  }
  else if (fHelicity == 1){
    locBeamHelicity->valid    = 1;
    locBeamHelicity->real_hel = 1;
  }
  else{
    locBeamHelicity->valid = 0;
  }
...
int  DBeamHelicity_factory::nextRand(uint32_t *bits){  // Apply the shift register algorithm to generate pseudorandom number (0,1)
  uint result=0;
...
int DBeamHelicity_factory::readParms(int source=0){    // Read setup etc from database or file
...
int DBeamHelicity_factory::setRunParms(int run){                               // Set parms for specific run
...
int DBeamHelicity_factory::printEvent(){
