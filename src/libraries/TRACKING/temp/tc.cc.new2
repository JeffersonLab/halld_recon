...
jerror_t DTrackCandidate_factory::evnt(JEventLoop *loop, uint64_t eventnumber)
{
  ...
  for (unsigned int j=0;j<cdc_backward_ids.size();j++){	  
	...
	if (TryToFlipDirection(schits,mom,pos)){
	  ...
	  // Get the cdc hits and find the hit with the largest radial distance
	  // from the beam line
	  vector<const DCDCTrackHit *>cdchits;
	  cdccan->GetT(cdchits);
	  stable_sort(cdchits.begin(), cdchits.end(), CDCHitSortByLayerincreasing);
	  double Rcdc_max=cdchits[cdchits.size()-1]->wire->origin.Perp();
	  
	  for (unsigned int i=0;i<fdctrackcandidates.size();i++){
	    if (num_fdc_cands_remaining==0) break;
	    if (forward_matches[i]==0){
	      const DTrackCandidate *fdccan=fdctrackcandidates[i];
	      //position of fdc candidate -- how does it compare to maximum
	      // cdc radius?
	      DVector3 fdcpos=fdccan->position();
	      if (Rcdc_max>fdcpos.Perp()) continue;
       
	      // Use MatchMethod1 algorithm	      
		...
		if (MakeCandidateFromMethod1(theta,segments,cdchits,cdccan)){
		  forward_matches[i]=1;
		  num_fdc_cands_remaining--;
		  cdc_backward_matches[j]=1;
		  if (DEBUG_LEVEL>0)
...
bool DTrackCandidate_factory::MatchMethod1(const DTrackCandidate *fdccan,
  ...
  if (cdc_fdc_match(p_fdc,p_cdc,diff_min)){
    ...
    if (MakeCandidateFromMethod1(mom.Theta(),segments,cdchits,
...
// candidates
bool DTrackCandidate_factory::MakeCandidateFromMethod1(double theta,vector<const DFDCSegment *>&segments,vector<const DCDCTrackHit*>&cdchits,const DTrackCandidate *cdccan){
