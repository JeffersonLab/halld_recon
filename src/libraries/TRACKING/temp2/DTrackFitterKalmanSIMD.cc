...
/*
   double DTrackFitterKalmanSIMD::fdc_drift_distance(double t,double myField.Bz) const {
...
DTrackFitterKalmanSIMD::DTrackFitterKalmanSIMD(JEventLoop *loop):DTrackFitter(loop){
  ...
  // Some useful values
   ...
   geom->GetFCALPosition(dFCALx,dFCALy,dFCALz);
   dFCALzBack=dFCALz+45.;
   // The following initializes ECAL position variables to large values so that
   // we don't make extroplations to the FCAL insert face if it is not present
   dECALz=dECALzBack=1000.;
   dECALsize=0.;
   if (geom->GetECALZ(dECALz)){
...
void DTrackFitterKalmanSIMD::ResetKalmanSIMD(void)
{
   ...
   myField.Bmag=0.;
   myField.dBxdx=0.,myField.dBxdy=0.,myField.dBxdz=0.,myField.dBydx=0.,myField.dBydy=0.,myField.dBydy=0.,myField.dBzdx=0.,myField.dBzdy=0.,myField.dBzdz=0.;
...
jerror_t DTrackFitterKalmanSIMD::CalcDerivAndJacobian(const DVector2 &xy,
   ...
   double q_over_pt=S(state_q_over_pt);
   double pt=fabs(1./q_over_pt);
   double q_over_pt_sinl=q_over_pt*sinl;
   ...
   double kq_over_pt=qBr2p*q_over_pt;
   double k_q_over_pt_sq_sinl=kq_over_pt*q_over_pt_sinl;
   ...
   if (CORRECT_FOR_ELOSS && fabs(dEdx)>EPS){  
      ...
      double E=sqrt(p_sq+mass2);
      double dEdx_over_E=dEdx/E;

      D1(state_q_over_pt)-=q_over_pt*E/p_sq*dEdx;
      J1(state_q_over_pt,state_q_over_pt)-=dEdx_over_E*(2.+3.*m2_over_p2);
      J1(state_q_over_pt,state_tanl)+=q_over_pt_sinl*cosl*dEdx_over_E*(1.+2.*m2_over_p2);
   }
...
jerror_t DTrackFitterKalmanSIMD::StepJacobian(const DVector2 &xy,
   ...
   double k_ds_q_over_pt_sq_sinl=kq_ds_over_pt*q_over_pt_sinl;
   double By_cosphi_minus_Bx_sinphi=myField.By*cosphi-myField.Bx*sinphi;
   double By_sinphi_plus_Bx_cosphi=myField.By*sinphi+myField.Bx*cosphi;
   ...
   if (CORRECT_FOR_ELOSS && fabs(dEdx)>EPS){  
      ...
      J(state_q_over_pt,state_tanl)+=q_over_pt*cosl*dE_over_E*sinl*(1.+2.*m2_over_p2);
   }
...
double DTrackFitterKalmanSIMD::GetEnergyVariance(double ds,
   ...
   cout << var << " " << K_rho_Z_over_A*fabs(ds)*Tmax*(one_over_beta2-0.5) << endl;
   return var;
}
