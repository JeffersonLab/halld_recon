...

{
	...
	TH2* locHist_CCALVsFCAL_TrigBit1 = (TH2*)gDirectory->Get("CCALVsFCAL_TrigBit1");
	...
	if(locHist_NumTriggers){
		...
		latex.DrawLatex(0.5, 0.7, "trig 1 ");
		latex.DrawLatex(0.72, 0.7, "trig 2 ");		
		latex.DrawLatex(0.95, 0.7, "trig 4  ");

		latex.DrawLatex(0.3, 0.50, "Triggers");

		TLine line;
		line.SetLineWidth(3.0);
		line.SetLineColor(kGray+2);
		line.DrawLine(0.05, 0.65, 0.95, 0.65);

		line.DrawLine(0.55, 0.4, 0.55, 0.75);
		line.DrawLine(0.75, 0.4, 0.75, 0.75);
		...
		latex.DrawLatex(0.45, 0.525, str);

		// trig 3
		sprintf(str, "%4.3g", h->GetBinContent(2,1));
		latex.DrawLatex(0.65, 0.525, str);

		// trig 4
		sprintf(str, "%4.3g", h->GetBinContent(4,1));
		latex.DrawLatex(0.85, 0.525, str);
		...
		latex.DrawLatex(0.45, 0.475, str);
		latex2.DrawLatex(0.45, 0.435, "of all trigs");

		// trig 2
		double Ntrig2 = h->GetBinContent(2,1);
		sprintf(str, "(%4.1f%%)", Ntrig2/all_trigs*100.0);
		latex.DrawLatex(0.65, 0.475, str);
		latex2.DrawLatex(0.65, 0.435, "of all trigs");

		// trig 4
		double Ntrig4 = h->GetBinContent(4,1);
		sprintf(str, "(%4.1f%%)", Ntrig4/all_trigs*100.0);
		latex.DrawLatex(0.85, 0.475, str);
		latex2.DrawLatex(0.85, 0.435, "of all trigs");

	}
	
	// CCAL vs. FCAL for Trig bit 1
	locCanvas->cd(0);
	if(locHist_CCALVsFCAL_TrigBit1 != NULL)
	{
		...
		locHist_CCALVsFCAL_TrigBit1->GetXaxis()->SetNdivisions(-404);
		locHist_CCALVsFCAL_TrigBit1->GetXaxis()->SetTitleSize(0.05);
		locHist_CCALVsFCAL_TrigBit1->GetYaxis()->SetTitleSize(0.04);
		locHist_CCALVsFCAL_TrigBit1->SetStats(0);
		locHist_CCALVsFCAL_TrigBit1->GetYaxis()->SetTitleOffset(2.0);
		locHist_CCALVsFCAL_TrigBit1->Draw("colz");

		sprintf(str, "%d entries", (uint32_t)locHist_CCALVsFCAL_TrigBit1->GetEntries());
	...
	if(locHist_L1bits_gtp!=NULL && locHist_L1bits_fp!=NULL)
	{
		...
		const char *bin_label[bin_number] = {"Main (1)", "FCal (2)", "PS (4)", "FCal LED (3)", "CCal LED (5)", "CCal Alpha (6)", "Random (12)", "BCal LED (9,10)"};
