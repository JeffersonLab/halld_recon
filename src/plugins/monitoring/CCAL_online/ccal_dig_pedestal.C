...

{
	...
	if( digPed ){
		...
		//digPed->GetXaxis()->SetRangeUser(200,600);
		c1->cd(1);
		digPed->Draw();
	}
	...
	if( digPedChan ){
		...
		digPedChan->GetYaxis()->SetRangeUser(300,500);
		digPedChan->GetYaxis()->SetTitleOffset(1.5);
		c1->cd(2);
		digPedChan->Draw();
	}
	...
	if( digPed2D && digOcc2D && digPed ){
		...
		digPed2DAvg->SetTitle("CCAL Pedestal - Average Pedestal");
		
		for( int x = 1; x <= digPed2DAvg->GetNbinsX(); ++x ){
			for( int y = 1; y <= digPed2DAvg->GetNbinsY(); ++y ){
				
				digPed2DAvg->SetBinContent(x, y, 
					digPed2DAvg->GetBinContent(x, y) - avgPed );
			}
		}
