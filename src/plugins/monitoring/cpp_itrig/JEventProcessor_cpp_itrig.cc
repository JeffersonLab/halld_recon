// $Id$
//
//    File: JEventProcessor_cpp_itrig.cc
...
// Use -Pcpp_itrig:MAKE_TREE=1 to produce tree output
...
//------------------
// JEventProcessor_cpp_itrig (Constructor)
//------------------
JEventProcessor_cpp_itrig::JEventProcessor_cpp_itrig()
{

}

//------------------
// ~JEventProcessor_cpp_itrig (Destructor)
//------------------
JEventProcessor_cpp_itrig::~JEventProcessor_cpp_itrig()
{
...
void JEventProcessor_cpp_itrig::Init()
{
  ...
  gDirectory->mkdir("cpp_itrig")->cd();

  hdiffs = new TH2I("errcount","Count of fa125 itrigger time errors; roc ; slot", 16, 1, 17, 17, 3, 20);

  hevents = new TH1I("num_events","Number of events", 1, 0.0, 1.0);

  for (int i=0; i<81; i++) rocmap[i] = 0;  // rocmap[rocid] = bin number for roc rocid in histogram
