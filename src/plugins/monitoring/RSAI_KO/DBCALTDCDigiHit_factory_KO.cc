// $Id$
//
//    File: DBCALTDCDigiHit_factory_KO.cc
...
// init
//------------------
jerror_t DBCALTDCDigiHit_factory_KO::init(void)
{
	// Tell JANA we don't own any of the objects in _data
	SetFactoryFlag( NOT_OBJECT_OWNER );

	FillEfficiencyMap("KO:BCAL_TDC_", BCAL_tdc_cell_eff);

	// Print list of boards that have an efficiency defined
	static std::once_flag flag;
	std::call_once( flag, [=](){
		cout << "BCAL TDC Digihit KO:" << endl;
		cout << "---------------" << endl;
		if( BCAL_tdc_cell_eff.empty() ) cout << "  <NONE> " << endl;
		for( auto p : BCAL_tdc_cell_eff ){
			auto &key = p.first;
			auto &eff = p.second;
...
// prefix.
//------------------
void DBCALTDCDigiHit_factory_KO::FillEfficiencyMap(string config_prefix, std::map< std::tuple<int,int,int,int>, double > &eff_map) {
...
//------------------
void DBCALTDCDigiHit_factory_KO::CheckRange(int module, int layer, int sector, int end) {
	...
	std::call_once(flag, [=]() {
		cout << endl;
		cout << "  Valid format for BCAL KO config. parameter is: KO:BCAL_TDC_MmLlSsEe" << endl;
...
// brun
//------------------
jerror_t DBCALTDCDigiHit_factory_KO::brun(jana::JEventLoop *eventLoop, int32_t runnumber)
{
	return NOERROR;
}
...
// evnt
//------------------
jerror_t DBCALTDCDigiHit_factory_KO::evnt(JEventLoop *loop, uint64_t eventnumber)
{
	vector<const DBCALTDCDigiHit*> bcaltdcdigihits;
	loop->Get(bcaltdcdigihits, "", false); // get original hits

	for( auto hit : bcaltdcdigihits ){
		auto key = std::make_tuple( hit->module, hit->layer, hit->sector, hit->end );
		if( BCAL_tdc_cell_eff.count(key) ){
			if( distribution(generator) <= BCAL_tdc_cell_eff[key]) _data.push_back( const_cast<DBCALTDCDigiHit*>(hit) );
		}else{
			_data.push_back( const_cast<DBCALTDCDigiHit*>(hit) );
		}
	}
...
// erun
//------------------
jerror_t DBCALTDCDigiHit_factory_KO::erun(void)
{
	return NOERROR;
}
...
// fini
//------------------
jerror_t DBCALTDCDigiHit_factory_KO::fini(void)
{
	return NOERROR;
}
