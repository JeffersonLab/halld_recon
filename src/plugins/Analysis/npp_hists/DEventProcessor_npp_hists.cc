...
extern "C"
{
	void InitPlugin(JApplication *locApplication)
	{
		...
		locApplication->AddProcessor(new DEventProcessor_npp_hists()); //register this plugin
		locApplication->AddFactoryGenerator(new DFactoryGenerator_npp_hists()); //register the factory generator
	}
} // "C"
...
// init
//------------------
jerror_t DEventProcessor_npp_hists::init(void)
{
	// This is called once at program startup.

	/*
	//OPTIONAL: Create an EventStore skim.  
	string locSkimFileName = "npp_hists.idxa";
...
// brun
//------------------
jerror_t DEventProcessor_npp_hists::brun(jana::JEventLoop* locEventLoop, int32_t locRunNumber)
{
...
// evnt
//------------------
jerror_t DEventProcessor_npp_hists::evnt(jana::JEventLoop* locEventLoop, uint64_t locEventNumber)
{
	...
	locEventWriterROOT->Fill_DataTrees(locEventLoop, "npp_hists");
	*/
	...
	for(size_t loc_i = 0; loc_i < locParticleCombos.size(); ++loc_i)
	{
		...
		if(locParticleCombo->Get_Reaction()->Get_ReactionName() != "npp_hists")
	...
	*/
	...
	for(size_t loc_i = 0; loc_i < locAnalysisResultsVector.size(); ++loc_i)
	{
		...
		if(locAnalysisResults->Get_Reaction()->Get_ReactionName() != "npp_hists")
	...
	*/
	...
	for(size_t loc_i = 0; loc_i < locAnalysisResultsVector.size(); ++loc_i)
	{
		...
		if(!locPassedParticleCombos.empty())
			locEventWriterREST->Write_RESTEvent(locEventLoop, "npp_hists"); //string is part of output file name
	}
	*/
	...
	for(size_t loc_i = 0; loc_i < locAnalysisResultsVector.size(); ++loc_i)
	{
		...
		japp->WriteLock("npp_hists.idxa"); //Lock is unique to this output file
		{
			dEventStoreSkimStream << locRunNumber << " " << locEventNumber << " " << locUniqueID << endl;
		}
		japp->Unlock("npp_hists.idxa");
	}
	*/
...
int DEventProcessor_npp_hists::Get_FileNumber(JEventLoop* locEventLoop) const
{
...
// erun
//------------------
jerror_t DEventProcessor_npp_hists::erun(void)
{
...
// fini
//------------------
jerror_t DEventProcessor_npp_hists::fini(void)
{
