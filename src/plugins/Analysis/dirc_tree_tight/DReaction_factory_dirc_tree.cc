...
jerror_t DReaction_factory_dirc_tree::evnt(JEventLoop* locEventLoop, uint64_t locEventNumber)
{
	...
	locReaction->Add_AnalysisAction(new DCutAction_MissingMassSquared(locReaction, true, -0.005, 0.005));

	// POST SELECTION
	locReaction->Add_AnalysisAction(new DHistogramAction_PID(locReaction, false,"PostSelection"));
	...
	// TIGHTER CUTS
	locReaction->Add_AnalysisAction(new DCutAction_InvariantMass(locReaction, 0, locPhiPIDs, true, 1., 1.04));
	locReaction->Add_AnalysisAction(new DCutAction_MissingMassSquared(locReaction, true, -0.005, 0.005));
