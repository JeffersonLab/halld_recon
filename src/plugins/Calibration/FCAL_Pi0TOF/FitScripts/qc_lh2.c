...
void qc_lh2(TString txtfile = "", TString path = "PDF/") {
  ...
  in.open(txtfile);
  //in.open("testing.txt");
  while ( in.good() && i < n_period) {
    TString LogLine = "";
    LogLine.ReadLine(in);
    Line = LogLine; 
    //                      gluex-fcal-checking-method0-lh2_period_5-1
    Line = Line.ReplaceAll("gluex-fcal-checking-method0-lh2_period_5-1", "");   
    Line = Line.ReplaceAll("gluex-fcal-checking-method1-lh2_period_5-1", "");   
    Line = Line.ReplaceAll("gluex-fcal-checking-method2-lh2_period_5-1", "");   
    Line = Line.ReplaceAll("gluex-cal-lh2_period_5-checking-method0", "");
    Line = Line.ReplaceAll("gluex-cal-lh2_period_5-checking-method1", "");
    //                      gluex_cal-lh2_period_5-checking-method2b
    Line = Line.ReplaceAll("gluex_cal-lh2_period_5-checking-method2b", "");
    Line = Line.ReplaceAll("ed-period-", "");
    Line = Line.ReplaceAll("edc", "");
    Line = Line.ReplaceAll("run", "");
    Line = Line.ReplaceAll("primex_runs_qc-", "");
    Line = Line.ReplaceAll("added", " ");
    Line = Line.ReplaceAll("/", " ");
    Line = Line.ReplaceAll("_", " ");
    Line = Line.ReplaceAll(".root", "");
    Line = Line.ReplaceAll(".pi0-fcal-skim", "");
    //if (!path.Contains("ed-period-")) 
    //sscanf(Line.Data(),"%*s %*s %s",tmp0);
    //else
    sscanf(Line.Data(),"%*s %*s %*s %s",tmp0);
    cout << "line " << Line << endl;
    int runnb = atof(tmp0);
    cout << runnb << endl;
    if ((runnb < 0 || runnb > n_period) || LogLine == "") continue;
    
    TFile * ifile = new TFile(LogLine);
    TH1F * h1D;
    if (txtfile.Contains("method0") || txtfile.Contains("method-0"))
      h1D = (TH1F *) ifile->Get("FCAL_Pi0HFA/Pi0Mass");
    else if (txtfile.Contains("method1") || txtfile.Contains("method-1"))
      h1D = (TH1F *) ifile->Get("FCAL_Pi0TOF/Pi0Mass");
    else if (txtfile.Contains("method2") || txtfile.Contains("method-2"))
      h1D = (TH1F *) ifile->Get("FCAL_Pi0log/Pi0Mass");
    if (h1D && h1D->GetEntries() > 0) {
