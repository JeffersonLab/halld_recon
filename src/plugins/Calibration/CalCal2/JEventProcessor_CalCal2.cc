...
void JEventProcessor_CalCal2::Init()
{
  ...
  gDirectory->mkdir("CalCal2")->cd();
  gDirectory->mkdir("ECAL")->cd();
  {
    hProtonECALGammaTimeDiff=new TH1F("hProtonECALGammaTimeDiff",";t_{0}-t(#gamma,ECAL) [ns]",200,-10,10);
    h2GammaMassECAL=new TH1F("h2GammaMassECAL","ECAL;m(2#gamma, ECAL only) [GeV]",500,0,1.);
    hEcalGains=new TH1F("hEcalGains",";ECAL gain distribution",200,0.75,1.25);
    hEcalGains2D=new TH2F("hEcalGains2D","ECAL gains;column;row",42,-1.5,40.5,
			  42,-1.5,40.5);
    hEcalCounts2D=new TH2F("hEcalCounts2D","ECAL counts;column;row",42,-1.5,40.5,
			  42,-1.5,40.5);
    h2ECALprob=new TH1F("h2ECALprob","Fit probability;CL",1000,0,1.);
  }
  gDirectory->cd("../");
  gDirectory->mkdir("FCAL")->cd();
  {
    hProtonFCALGammaTimeDiff=new TH1F("hProtonFCALGammaTimeDiff",";t_{0}-t(#gamma,FCAL) [ns]",200,-10,10);
    h2GammaMassFCAL=new TH1F("h2GammaMassFCAL","FCAL;m(2#gamma, FCAL only) [GeV]",500,0,1.);
    hFcalGains=new TH1F("hFcalGains",";FCAL gain distribution",200,0.75,1.25);
    hFcalGains2D=new TH2F("hFcalGains2D","FCAL gains;column;row",61,-1.5,59.5,
			  61,-1.5,59.5);
    hFcalCounts2D=new TH2F("hFcalCounts2D","FCAL counts;column;row",61,-1.5,59.5,
			  61,-1.5,59.5);
    h2FCALprob=new TH1F("h2FCALprob","Fit probability;CL",1000,0,1.);
  }
  gDirectory->cd("../");
  gDirectory->mkdir("ECALFCAL")->cd();
  {
    h2GammaMassECALFCAL=new TH1F("h2GammaMassECALFCAL",";m(2#gamma, ECAL+FCAL) [GeV]",500,0,1.);
    hECALFCALprob=new TH1F("hECALFCALprob","Fit probability;CL",1000,0,1.);
  }
  gDirectory->cd("../..");
  ...
  ECAL_SHOWER_MIN=1.5; // GeV
  app->SetDefaultParameter("CalCal:ECAL_SHOWER_MIN",ECAL_SHOWER_MIN);
  FCAL_SHOWER_MIN=1.; // GeV
  app->SetDefaultParameter("CalCal:FCAL_SHOWER_MIN",FCAL_SHOWER_MIN);
  EHIT_MIN=0.35; // GeV
  app->SetDefaultParameter("CalCal:EHIT_MIN",EHIT_MIN);
  TIME_CUT=2.;
  app->SetDefaultParameter("CalCal:TIME_CUT",TIME_CUT);
  USE_DVERTEX=false;
  app->SetDefaultParameter("CalCal:USE_DVERTEX",USE_DVERTEX);

  // ECAL hit resolution parameters
  ECAL_VAR_E_PAR0=1e-4;
  ECAL_VAR_E_PAR1=0.;
  ECAL_VAR_E_PAR2=0.;
  ...
  // Scale factors for mass goal to account for incomplete geometric coverage,
  // impact of thresholds, etc.
  ECAL_SCALE_FACTOR=0.96;
...
void JEventProcessor_CalCal2::Process(const std::shared_ptr<const JEvent> &event)
{
  ...
  DVector3 vertex;
  double t0;
  if (USE_DVERTEX){  //skimmed files already contain vertex information
    auto myDVertex=event->GetSingle<DVertex>();
    if (myDVertex==NULL) return;

    vertex=myDVertex->dSpacetimeVertex.Vect();
    t0=myDVertex->dSpacetimeVertex.T();
  }
  else {
    // Look for one track
    ...
    if (triggers[0]->Get_L1TriggerBits()==0) return;
    ...
    if (hyp->momentum().Theta()*180/M_PI<10.) return;
  ...
  // Collect vectors of ECAL/FCAL hits that satisfy a time cut and some
  ...
  for (size_t i=0;i<ecalshowers.size();i++){
    ...
    if (ecalshowers[i]->E<ECAL_SHOWER_MIN) continue;
  ...
  for (size_t i=0;i<fcalshowers.size();i++){
    // Apply minimum shower energy cut
    if (fcalshowers[i]->getEnergy()<FCAL_SHOWER_MIN) continue;
...
// angle is negligible
double JEventProcessor_CalCal2::ApplyConstraint(double scale1,double scale2,
  ...
  double mass_sq=scale1*scale2*pow(ParticleMass(Pi0),2.);
...
void JEventProcessor_CalCal2::EcalAnalysis(pair<DVector3,vector<const DECALHit*>>&cluster1,pair<DVector3,vector<const DECALHit*>>&cluster2){
  ...
  if (mass>PI0_LO_CUT && mass<PI0_HI_CUT){
    double chisq=ApplyConstraint(ECAL_SCALE_FACTOR,ECAL_SCALE_FACTOR,
				 one_minus_costheta12,E1sum,V1sum,E2sum,V2sum,
				 E1vec,V1vec,E2vec,V2vec);
    unsigned int ndf=E1vec.size()+E2vec.size()+1;
    double CL=TMath::Prob(chisq,ndf);
    h2ECALprob->Fill(CL);
...
void JEventProcessor_CalCal2::FcalAnalysis(pair<DVector3,vector<const DFCALHit*>>&cluster1,pair<DVector3,vector<const DFCALHit*>>&cluster2){
  ...
  // Apply the pi0 constraint
  if (mass>PI0_LO_CUT && mass<PI0_HI_CUT){
    double chisq=ApplyConstraint(FCAL_SCALE_FACTOR,FCAL_SCALE_FACTOR,
				 one_minus_costheta12,E1sum,V1sum,E2sum,V2sum,
				 E1vec,V1vec,E2vec,V2vec);
    unsigned int ndf=E1vec.size()+E2vec.size()+1;
    double CL=TMath::Prob(chisq,ndf);
...
void JEventProcessor_CalCal2::EcalFcalAnalysis(pair<DVector3,vector<const DECALHit*>>&ecluster,pair<DVector3,vector<const DFCALHit*>>&fcluster){
  ...
  // Apply the pi0 constraint
  if (mass>PI0_LO_CUT && mass<PI0_HI_CUT){
    double chisq=ApplyConstraint(ECAL_SCALE_FACTOR,FCAL_SCALE_FACTOR,
				 one_minus_costheta12,E1sum,V2sum,E2sum,V2sum,
				 E1vec,V1vec,E2vec,V2vec);
    unsigned int ndf=E1vec.size()+E2vec.size()+1;
    double CL=TMath::Prob(chisq,ndf);
    hECALFCALprob->Fill(CL);
